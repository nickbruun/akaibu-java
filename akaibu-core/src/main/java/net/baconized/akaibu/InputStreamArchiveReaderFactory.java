...
public class InputStreamArchiveReaderFactory {
     ...
     */
    public static InputStreamArchiveReader create(InputStream stream) throws InvalidCompressionException, InvalidArchiveException, UnsupportedVersionException, IOException {
        ...
        if ((header[0] != ArchiveFormat.magicHeader[0]) ||
            ...
            throw new InvalidArchiveException(String.format("archive has invalid magic header"));

        if (header[4] != 1)
            throw new UnsupportedVersionException(String.format("unsupported archive version: %d", header[4]));
        ...
        default:
            throw new InvalidCompressionException(String.format("invalid compression method: %s", compression));
        }
    }
}
